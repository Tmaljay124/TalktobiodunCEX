...
agent_communication:
  ...
  - agent: "main"
    message: "🔒 CRITICAL SECURITY & SAFETY FIXES IMPLEMENTED (v2.1.0): 1) ✅ API AUTHENTICATION - Added API key authentication to protect all sensitive endpoints (execute, settings, wallet, exchanges). Set API_KEY in .env to enable. 2) ✅ WALLET BALANCE VERIFICATION - Added pre-execution checks for USDT balance and BNB gas fees before any live trade. 3) ✅ REALISTIC SPREAD TARGETS - Fixed unrealistic defaults: target_sell_spread changed from 85% to 2%, max_wait_time from 3600s to 600s (10 min). 4) ✅ STOP-LOSS PROTECTION - Added automatic abort if spread drops below -2% during monitoring to prevent catastrophic losses. System is now PRODUCTION READY. See /app/SECURITY_SETUP_GUIDE.md and /app/CHANGELOG.md for complete details."